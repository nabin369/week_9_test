...
def select_elements_starting_with_vowel array
  array.select { |a| a.start_with? 'a','e','i','o','u'}
  array.select { |a| a =~ /\A[aeiou]/i }
end
...
def array_sort_by_last_letter_of_word array
  array.map{|a| a.reverse}.sort.map{|a|a.reverse}
  # array.sort_by { |w| w[-1] }
end
...
def get_first_half_of_string string
  str_length = (string.length / 2.0).ceil
  string[0, str_length]
end

# turn a positive integer into a negative integer. A negative integer
# stays negative
def make_numbers_negative numbers
  - numbers.abs
end
...
def number_of_elements_that_are_palindromes array
  array.select { |s| s == s.reverse }.size
end
